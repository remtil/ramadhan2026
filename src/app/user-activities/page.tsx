...
export default async function DashboardPage() {
  ...
  const completedToday = await db
    ...
    .where(
      and(
        eq(userActivity.userId, TEST_USER_ID),
  ...
  const allTimeHistory = await db
    .select()
    .from(userActivity)
    .where(eq(userActivity.userId, TEST_USER_ID));
  const currentStreak = await calculateUserStreak(TEST_USER_ID, serverToday);
  
  const completedIds = completedToday.map((record) => record.activityId);
  const totalPointsAllTime = allTimeHistory.reduce((sum, r) => sum + r.pointsEarned, 0);
  const todaysPoints = completedToday.reduce((sum, r) => sum + r.pointsEarned, 0);
  const totalAvailablePoints = allActivities.reduce((sum, a) => sum + a.points, 0);
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <header className="space-y-4">
          <div className="flex justify-between items-end">
            ...
            <div className="text-right">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Today's Progress</p>
              <p className="text-xl font-bold text-emerald-400">{completedToday.length}/{allActivities.length} <span className="text-sm text-slate-500 font-normal">activities done</span></p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-[#12161f] border border-slate-800 rounded-xl p-5 hover:border-blue-500/30 transition-colors">
            <div className="flex justify-between items-start mb-4">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Points</p>
              <span>💎</span>
            </div>
            <h2 className="text-3xl font-black text-blue-400">{totalPointsAllTime}</h2>
            <p className="text-xs text-slate-500 mt-1">All time</p>
          </div>

          <div className="bg-[#12161f] border border-emerald-500/30 rounded-xl p-5 shadow-[0_0_15px_rgba(16,185,129,0.05)]">
            <div className="flex justify-between items-start mb-4">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Today's Points</p>
              <span>🎯</span>
            </div>
            <h2 className="text-3xl font-black text-emerald-400">{todaysPoints}</h2>
            <p className="text-xs text-slate-500 mt-1">of {totalAvailablePoints} available</p>
          </div>
          ...
          <div className="bg-[#12161f] border border-orange-500/30 rounded-xl p-5 shadow-[0_0_15px_rgba(249,115,22,0.05)]">
            ...
            <h2 className="text-3xl font-black text-orange-400">{currentStreak}d</h2>
          ...
          <div className="bg-[#12161f] border border-slate-800 rounded-xl p-5 hover:border-green-500/30 transition-colors">
            ...
            <h2 className="text-3xl font-black text-white">{allTimeHistory.length}</h2>
        ...
        <div>
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-xs font-medium bg-[#1a202c] text-slate-400 px-3 py-1 rounded-md">{serverToday}</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {allActivities.map((activity) => {
              ...
              return (
                <div key={activity.id} className={`bg-[#12161f] border ${isDone ? 'border-emerald-500/50' : 'border-slate-800 hover:border-slate-700'} rounded-xl p-4 flex items-center justify-between group transition-all`}>
                  <div className="flex items-center gap-4 opacity-100">
                    ...
                    <div>
                      ...
                      <h3 className="font-bold text-slate-200">{activity.title}</h3>
                      <p className="text-xs text-slate-500">{activity.description}</p>
                    </div>
                  </div>

                  <div className="flex flex-col items-end gap-2">
                    <span className="text-xs font-bold text-amber-400">+{activity.points} pts</span>
                    {/* The Circular Checkbox Component */}
                    <CompleteButton 
                      userId={TEST_USER_ID} 
